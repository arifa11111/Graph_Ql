...
export const Upload = () => {
    ...
    const [label,setLabel] = useState<string>('Apply')
    const handleClick = () => {
        setOpen(false)
        setSuccess(true)
        label === 'Apply' ? setLabel('Applied') : setLabel('Apply')
    }
    return (
        <>
            {success && <UploadSuccess />}
            <ButtonComponent label={label} variant={'contained'} classing={'next'} onClick={() => setOpen(true)} style={{
